...
db = MySQLDatabase(
    'tienda',
    user='root',
    password=config('DBPASSWORD'),      # Variable de entorno (averiguar cómo se hace en Windows) - Para ubuntu es: export DBPASSWORD=root
    port=3306,
    host='localhost'
)
...
class User(Model): # Tablas
    email = TextField()
    password = TextField() # no es buena práctica poner así una contraseña
